...
extern "C" void bpftime_agent_main(const gchar* data, gboolean* stay_resident) {
    {
        int expected = 0;
        if (!__atomic_compare_exchange_n(&initialized, &expected, 1, false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST)) {
    ...
    try {
        ...
        SPDLOG_ERROR("Unable to initialize shared memory: {}", ex.what());
        return;
    }
    ...
    ctx_holder.ctx.register_attach_impl(
        {ATTACH_SYSCALL_TRACE}, std::move(syscall_trace_impl), [](const std::string_view& sv, int& err) {
            std::unique_ptr<attach_private_data> priv_data = std::make_unique<syscall_trace_attach_private_data>();
            if (int e = priv_data->initialize_from_string(sv); e < 0) {
                err = e;
    ...
    ctx_holder.ctx.register_attach_impl(
        {ATTACH_UPROBE, ATTACH_URETPROBE, ATTACH_UPROBE_OVERRIDE, ATTACH_UREPLACE},
        std::make_unique<attach::frida_attach_impl>(), [](const std::string_view& sv, int& err) {
            std::unique_ptr<attach_private_data> priv_data = std::make_unique<frida_attach_private_data>();
            if (int e = priv_data->initialize_from_string(sv); e < 0) {
                err = e;
                return std::unique_ptr<attach_private_data>();
            }
    ...
    *stay_resident = TRUE;
    ...
    try {
        res = ctx_holder.ctx.init_attach_ctx_from_handlers(runtime_config);
        if (res != 0) {
...
extern "C" int64_t syscall_callback(int64_t sys_nr, int64_t arg1, int64_t arg2, int64_t arg3, int64_t arg4,
                                    int64_t arg5, int64_t arg6) {
    return bpftime::attach::global_syscall_trace_attach_impl.value()->dispatch_syscall(sys_nr, arg1, arg2, arg3, arg4,
                                                                                       arg5, arg6);
}

extern "C" void _bpftime__setup_syscall_trace_callback(syscall_hooker_func_t* hooker) {
    orig_hooker = *hooker;
